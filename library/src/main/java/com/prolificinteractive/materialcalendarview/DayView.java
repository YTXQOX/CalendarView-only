...
class DayView extends CheckedTextView {
    ...
    private int selectionColor = Color.argb(111, 151, 179, 217);
    ...
    protected void onDraw(@NonNull Canvas canvas) {
        if (customBackground != null) {
            Rect myTempRect = null;
            if (isFirst) {
                myTempRect = new Rect(tempRect.left, tempRect.top + (int) (tempRect.bottom * (1 - RATIO) / 2), tempRect.right, tempRect.bottom - (int) (tempRect.bottom * (1 - RATIO) / 2));
                myTempRect.left = myTempRect.right / 2;
            } else if (isLast) {
                myTempRect = new Rect(tempRect.left, tempRect.top + (int) (tempRect.bottom * (1 - RATIO) / 2), tempRect.right, tempRect.bottom - (int) (tempRect.bottom * (1 - RATIO) / 2));
                myTempRect.right = myTempRect.right / 2;
            } else if (isMiddle) {
                myTempRect = new Rect(tempRect.left, tempRect.top + (int) (tempRect.bottom * (1 - RATIO) / 2), tempRect.right, tempRect.bottom - (int) (tempRect.bottom * (1 - RATIO) / 2));
            }

            customBackground.setBounds(myTempRect == null ? tempRect : myTempRect);
            customBackground.setState(getDrawableState());
            customBackground.draw(canvas);
        }

        if (Build.VERSION.SDK_INT == Build.VERSION_CODES.LOLLIPOP) {
//        mCircleDrawable.setBounds(circleDrawableRect.left + (int) (circleDrawableRect.right * (1 - RATIO) / 2),
//                circleDrawableRect.top + (int) (circleDrawableRect.bottom * (1 - RATIO) / 2),
//                circleDrawableRect.right - (int) (circleDrawableRect.right * (1 - RATIO) / 2),
//                circleDrawableRect.bottom - (int) (circleDrawableRect.bottom * (1 - RATIO) / 2));
        } else {
    ...
    private void regenerateBackground() {
        if (isFirst || isLast || isMiddle) {
            customBackground = generateCustomBackground(selectionColor, fadeTime, circleDrawableRect, isFirst, isLast, isMiddle);
        } else {
        ...
        if (selectionDrawable != null) {
            ...
            mCircleDrawable = generateBackground(selectionColor, fadeTime, circleDrawableRect, isFirst, isLast, isMiddle);
...
//        }
    ...
    private static Drawable generateBackground(int color, int fadeTime, Rect bounds, boolean isFirst, boolean isLast, boolean isMiddle) {
        ...
        color = isMiddle ? Color.TRANSPARENT : color;
...
//        }
    ...
    // 背景
    private static Drawable generateCustomBackground(int color, int fadeTime, Rect bounds, boolean isFirst, boolean isLast, boolean isMiddle) {
        StateListDrawable drawable = new StateListDrawable();
        drawable.setExitFadeDuration(fadeTime);
        color = isFirst | isLast | isMiddle ? Color.argb(111, 151, 179, 217) : color;
        drawable.addState(new int[]{android.R.attr.state_checked}, generateRectDrawable(color));
//        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
//            drawable.addState(new int[]{android.R.attr.state_pressed}, generateRippleDrawable1(color, bounds));
//        } else {
...
//        }
    ...
    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    private static Drawable generateRippleDrawable1(final int color, Rect bounds) {
        ColorStateList list = ColorStateList.valueOf(color);
    ...
    private void calculateBounds(int width, int height) {
        ...
        if (width >= height) {
            ...
            if (Build.VERSION.SDK_INT == Build.VERSION_CODES.LOLLIPOP) {
                ...
                circleDrawableRect.left = circleDrawableRect.left + (int) (circleDrawableRect.right * (1 - RATIO) / 2);
                circleDrawableRect.top = circleDrawableRect.top + (int) (circleDrawableRect.bottom * (1 - RATIO) / 2);
                circleDrawableRect.right = circleDrawableRect.right - (int) (circleDrawableRect.right * (1 - RATIO) / 2);
                circleDrawableRect.bottom = circleDrawableRect.bottom - (int) (circleDrawableRect.bottom * (1 - RATIO) / 2);
            }
        } else {
