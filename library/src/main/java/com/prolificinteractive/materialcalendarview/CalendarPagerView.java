...
abstract class CalendarPagerView extends ViewGroup implements View.OnClickListener {
    ...
    public void setSelectedDates(Collection<CalendarDay> dates) {
        ...
        boolean startFlag = dates.size() >= 2;
        CalendarDay[] calendarDays = dates.toArray(new CalendarDay[dates.size()]);
        Arrays.sort(calendarDays, new DayComparator());
        CalendarDay firstSelectedDate = calendarDays[0];
        ...
        for (DayView dayView : dayViews) {
            ...
            if (checked & startFlag) {
                if (day.equals(firstSelectedDate)) {
                    dayView.setFirst();
                } else if (day.equals(lastSelectedDate)) {
                    dayView.setLast();
                } else {
                    dayView.setMiddle();
                }
...
//            Log.i("this.date-->", this.date.toString());
//            Log.i("calendarDay0-->", CalendarPagerAdapter.calendarDayLast.toString());
//            Log.i("calendarDayLast-->", CalendarPagerAdapter.calendarDayLast.toString());
//
//            this.selectionColor = Color.parseColor("#3b87ee");
//        } else {
//            this.selectionColor = Color.parseColor("#97b3d9");
//        }
        postInvalidate();
    }
